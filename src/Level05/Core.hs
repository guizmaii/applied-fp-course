...
prepareAppReqs = DB.initDB (dbFilePath Conf.firstAppConfig) <&> mapLeft DBInitErr
...
app db rq cb =
  let prg = mkRequest rq >>= handleRequest db
  in runAppM prg <&> either mkErrorResponse id >>= cb
