...
instance Functor (AppM e) where
  ...
  fmap f (AppM ioea) = AppM $ (fmap . fmap) f ioea
...
instance Applicative (AppM e) where
  ...
  (<*>) (AppM ioeaf) (AppM ioea) = AppM $ (ioeaf <&> (<*>)) <*> ioea
...
instance MonadError e (AppM e) where
  ...
  catchError (AppM ioea) r = AppM $ ioea >>= either (runAppM . r) (pure . pure)
...
instance Bifunctor AppM where
  ...
  bimap leftF rightF (AppM ioea) = AppM $ ioea <&> bimap leftF rightF
